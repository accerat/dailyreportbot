...
async function postTriggerIntakeMessages(client, project, report, authorId) {
  for (const t of report.triggers || []) {
    ...
    try {
      ...
      if (!ch || typeof ch.send !== 'function') { console.warn('trigger post skipped: not a text channel', channelId); continue; }
      await ch.send(
...
export function wireInteractions(client) {
  ...
  async function showPanel(msg, project) {
    ...
    try {
      const _rows = [row1, row2, row3, row4].filter(r => r && r.components && r.components.length >= 1 && r.components.length <= 5);
      await msg.reply({ embeds: [embed], components: _rows });
    } catch {
      const _rows = [row1, row2, row3, row4].filter(r => r && r.components && r.components.length >= 1 && r.components.length <= 5);
      await msg.channel.send({ embeds: [embed], components: _rows });
    }
  }
